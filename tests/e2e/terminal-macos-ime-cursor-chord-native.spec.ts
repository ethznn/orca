...
// Chinese, both scripts, and both need their parent enabled first for the same reason Kotoeri
// does. Measured 2026-08-09 on the Command-release fix: both swallow the chord and keep the
// preedit, and the byte drains at the commit — the Japanese shape, not the Korean one.
const SIMPLIFIED_PINYIN_ID = 'com.apple.inputmethod.SCIM.ITABC'
const SIMPLIFIED_PINYIN_PARENT_ID = 'com.apple.inputmethod.SCIM'
const TRADITIONAL_ZHUYIN_ID = 'com.apple.inputmethod.TCIM.Zhuyin'
const TRADITIONAL_ZHUYIN_PARENT_ID = 'com.apple.inputmethod.TCIM'
...
test.describe('Native macOS IME cursor chords during composition @headful', () => {
  ...
  /**
   * Chinese, both scripts, both chords. Measured on this build 2026-08-09 (ladder traces under
   * scratchpad/ime-traces/round4), driven with the modifier as its own key event — every earlier
   * Chinese capture folded it into the arrow's flags, which produces no modifier press or release
   * at all, and so could not see the Command release that ends the gesture.
   *
   ...
   *     still marked composing. Both those cells are green only because of that release being
   *     honoured; before it the byte was dropped and the line arrived without it.
  ...
  for (const cell of [
    ...
    test(`${cell.script}: ${cell.chord} during composition puts its byte after the commit`, async ({
      ...
      try {
        ...
        pressChord(processId, KEY.left, cell.modifier)
        ...
        // Anchored, so the byte cannot also appear inside the committed text: a copy ahead of it
        // would mean the chord was never queued, and a second copy that both releases fired.
